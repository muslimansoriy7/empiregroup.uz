  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 4, color: 'var(--color-ink)' }}>CRM Dashboard</h1>
        <p style={{ color: 'var(--color-mute)', fontSize: 14 }}>Pipeline, leadlar va biznes analitika</p>
      </div>
      ...
      <div className="crm-stats">
        <Link href="/admin/leads?status=new" style={{ textDecoration: 'none' }}>
          <div className="crm-stat">
            ...
            <div className="crm-stat-value" style={{ color: stats.new > 0 ? '#ef6b5a' : 'var(--color-ink)' }}>{stats.new}</div>
        ...
        <div className="crm-stat">
          ...
          <div className="crm-stat-value" style={{ color: 'var(--color-link)' }}>{money(stats.totalValue)}</div>
      ...
      {pipelineTotal > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 12, color: 'var(--color-mute)', marginBottom: 6, fontWeight: 600, letterSpacing: '.06em', textTransform: 'uppercase' }}>PIPELINE FUNNEL</div>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 6, fontSize: 12, color: 'var(--color-mute)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        <div>
                ...
                {overdueFollowUps.slice(0, 5).map(l => (
                  <Link key={l.id} href={`/admin/leads/${l.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                    <div style={{ padding: '8px 12px', background: 'rgba(220,38,38,.06)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid rgba(220,38,38,.15)' }}>
                      <div>
                        <strong style={{ fontSize: 14, color: 'var(--color-ink)' }}>{l.name}</strong>
                        {l.company && <span style={{ fontSize: 12, color: 'var(--color-mute)', marginLeft: 8 }}>{l.company}</span>}
                      </div>
          ...
          {upcomingFollowUps.length > 0 && (
            <div className="crm-section" style={{ borderColor: 'color-mix(in srgb,var(--color-link) 20%,transparent)', background: 'color-mix(in srgb,var(--color-link) 7%,transparent)' }}>
              <h3 style={{ color: 'var(--color-link)' }}>⏰ Yaqin follow-up ({upcomingFollowUps.length})</h3>
              <div className="crm-list">
                {upcomingFollowUps.slice(0, 5).map(l => (
                  <Link key={l.id} href={`/admin/leads/${l.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                    <div style={{ padding: '8px 12px', background: 'color-mix(in srgb,var(--color-link) 7%,transparent)', borderRadius: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid color-mix(in srgb,var(--color-link) 14%,transparent)' }}>
                      <div>
                        <strong style={{ fontSize: 14, color: 'var(--color-ink)' }}>{l.name}</strong>
                        {l.service && <span style={{ fontSize: 12, color: 'var(--color-mute)', marginLeft: 8 }}>{l.service}</span>}
                      </div>
                      <span style={{ fontSize: 12, color: 'var(--color-link)', fontWeight: 600 }}>{fmt(l.next_follow_up)}</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="crm-section">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
              <h3 style={{ margin: 0, color: 'var(--color-ink)' }}>So'nggi leadlar</h3>
              <Link href="/admin/leads" style={{ fontSize: 13, color: 'var(--color-link)', textDecoration: 'none', fontWeight: 600 }}>Hammasi →</Link>
            </div>
            {recentLeads.length === 0 ? (
              <p style={{ fontSize: 14, color: 'var(--color-mute)', textAlign: 'center', padding: 20 }}>Saytdan lead kutilmoqda...</p>
            ) : (
              <div className="crm-list">
                {recentLeads.map(lead => (
                  <Link key={lead.id} href={`/admin/leads/${lead.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                    <div className="crm-lead-row" style={{ gridTemplateColumns: '1fr auto', border: '1px solid var(--color-hairline-soft)' }}>
                      <div>
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 3 }}>
                          <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-ink)' }}>{lead.name || '—'}</span>
                          {lead.company && <span style={{ fontSize: 12, color: 'var(--color-mute)' }}>({lead.company})</span>}
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--color-mute)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <div>
          ...
          {topSources.length > 0 && (
            <div className="crm-section">
              <h3 style={{ color: 'var(--color-ink)' }}>📊 Lead manbalari</h3>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {topSources.map(([src, count]) => (
                  <div key={src} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '6px 0' }}>
                    <span style={{ fontSize: 14, fontWeight: 500, textTransform: 'capitalize', color: 'var(--color-ink)' }}>{src}</span>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 60, height: 6, borderRadius: 3, background: 'var(--color-hairline-soft)', overflow: 'hidden' }}>
                        <div style={{ width: `${(count / (topSources[0]?.[1] || 1)) * 100}%`, height: '100%', background: 'var(--color-link)', borderRadius: 3 }} />
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 700, minWidth: 20, textAlign: 'right', color: 'var(--color-ink)' }}>{count}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* Quick actions */}
          <div className="crm-section">
            <h3 style={{ color: 'var(--color-ink)' }}>⚡ Tezkor harakatlar</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
              {[
                ...
                <Link key={href} href={href} style={{ display: 'block', padding: '8px 12px', background: 'var(--color-hairline-soft)', border: '1px solid var(--color-hairline)', borderRadius: 8, fontSize: 13, textDecoration: 'none', color: 'var(--color-ink)', fontWeight: 500 }}>
                  {label}
                </Link>
              ))}
            </div>
