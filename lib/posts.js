...
export async function getPublishedPosts({ category } = {}) {
  return safe(async () => {
    ...
    // The index renders cards, not articles — pulling every post's full
    // markdown to show a title and one line of excerpt is wasted bandwidth.
    const CARD_COLUMNS = 'id, slug, title_uz, title_ru, excerpt_uz, excerpt_ru, category, cover_url, published_at, updated_at';
    let q = supabase.from('posts').select(CARD_COLUMNS).eq('status', 'published').order('published_at', { ascending: false });
